...
export const PopupUI = () => {
    ...
    // Read the popup from storage and set the state
    useEffect(() => {
        chrome.storage.local.get(['popupData'], (result) => {
            const data = result.popupData as PopupData;
            console.log(data);
            setTextarea(data.message);
            setHour(data.hourToWait);
            setSeconds(data.maximumInterval);
        });
    }, [])

    useEffect(() => {
        // send a sendMessage to the contentScript
        if (triggerClock) {
            chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
                chrome.tabs.sendMessage(tabs[0].id, {message: "startClock", hour, seconds}, function(response) {
    ...
    useEffect(() => {
        ...
        const data: PopupData = {
            message: textarea,
            hourToWait: hour,
            maximumInterval: seconds
        }
        chrome.storage.local.set({popupData: data}, () => {
            console.log('Data saved');
        });
    }, [textarea, hour, seconds])
    return (
        <div className="container__wrapper">
            ...
            <div className="container">
                <TextAreaComponent message={textarea} setMessage={setTextarea}/>
                ...
                <Slider
                    ...
                    valueLabelDisplay={"on"}
                    size={'small'}
                    onChange={(e, value) => setSeconds(value as number)}
                    max={60}
                    color="primary"
                />
                ...
                <div className="action-btns">
                    { triggerClock && (
                        <button onClick={e => setTriggerClock(false)} className="action-btn action-btn__purple">Stop</button>
                    )}
                    { !triggerClock && (
                        <button onClick={e => setTriggerClock(true)} className="action-btn action-btn__purple">Start</button>
                    )}
